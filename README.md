...
需要在移动端继续进行 **PDF 文字直接编辑** 或 **PDF / Word / PowerPoint 本地格式转换**，可以使用增强版 **MyPDF Pro**。

<p align="center">
  <a href="https://github.com/g-o-d-v/MyPDF-Pro"><strong>了解 MyPDF Pro</strong></a>
  ·
  <a href="https://github.com/g-o-d-v/MyPDF-Pro/releases/latest"><strong>下载最新 Pro 版本</strong></a>
</p>

MyPDF Pro 在基础阅读与文档处理能力之外，重点增加：

- 对支持的文本层 PDF 进行文字直接编辑；
- PDF → Word、PDF → PowerPoint；
- Word → PDF、PowerPoint → PDF；
- 尽量保留或重建可编辑文字与原始版式；
- 本地 OCR 扫描与可搜索 PDF 处理等增强能力。
...
> 本仓库仍持续提供 MyPDF 基础版。Pro 版是独立产品，不影响基础版已有开源功能的使用、下载和维护。
...
当前 `v1.0.2` 重点修复 PDF 合并独立进程误访问 Firebase / Crashlytics 导致任务无法继续的问题、恢复自动阅读位置记录，并加入 Android 系统 PDF 打开方式注册。
...
- PDF 阅读与页面浏览；
- 自动记录每个 PDF 的阅读页和纵向滚动位置，下次打开时自动恢复；
- 安装后可注册为系统 PDF 打开方式，从文件管理器或其他应用直接选择 **MyPDF** 打开 PDF；
- 顶部状态栏居中显示文件名与当前页码进度；
- 可选的实验性阅读控制栏（默认关闭）：纵向 / 横向翻页、翻页动画、观看进度；
- 横向模式支持左右区域翻页、中间区域显示或隐藏菜单；
- 条漫优先使用纵向阅读，并可使用百分比观看进度；
- 文本层长按选择和复制；
- 文件收藏；
- 文件详情查看；
- 支持较长页面和长图类文档；
...
> 实验性阅读控制栏默认关闭。部分漫画或特殊比例 PDF 在横向模式下可能出现较多留白，观看体验可能不如纵向模式。
...
- 另存所选页面；
- 合并多个 PDF；
- 使用独立进程执行部分高负载 PDF 操作，降低合并任务对阅读界面的影响。
...
- 从其他应用通过临时 `content://` URI 打开的 PDF，其长期访问能力取决于来源应用授予的 URI 权限；
- 实验性横向阅读不会自动切割双栏或跨页漫画，特殊比例页面可能出现留白；
